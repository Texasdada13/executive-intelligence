"""File Analyzer for CSV/Excel Data Analysis - Executive Intelligence"""
...
class FileAnalyzer:
    ...
    # Executive/business-specific metric patterns
    EXECUTIVE_METRIC_PATTERNS = {
        'revenue': ['revenue', 'sales', 'income', 'earnings', 'arr', 'mrr'],
        'profit': ['profit', 'margin', 'ebitda', 'net income', 'gross profit'],
        'growth': ['growth', 'yoy', 'qoq', 'change', 'increase', 'decrease'],
        'customers': ['customer', 'client', 'account', 'user', 'subscriber'],
        'churn': ['churn', 'attrition', 'retention', 'cancellation'],
        'employees': ['employee', 'headcount', 'staff', 'team', 'fte'],
        'goals': ['goal', 'target', 'objective', 'okr', 'kpi'],
        'status': ['status', 'progress', 'complete', 'on track', 'at risk'],
        'period': ['period', 'quarter', 'month', 'year', 'q1', 'q2', 'q3', 'q4'],
        'department': ['department', 'team', 'function', 'division', 'unit'],
    }
    ...
    def _analyze_csv(self, content: bytes, filename: str, delimiter: str = ',') -> FileAnalysisResult:
        ...
        detected_metrics = self._detect_executive_metrics(columns, rows)
    ...
    def _analyze_json(self, content: bytes, filename: str) -> FileAnalysisResult:
        ...
        if isinstance(data, list):
            ...
            if 'data' in data:
                ...
                rows = data['goals'][:self.MAX_ROWS]
            elif 'metrics' in data:
                rows = data['metrics'][:self.MAX_ROWS]
            else:
                rows = [data]
        else:
            rows = []
    ...
    def _detect_executive_metrics(
        self,
        columns: List[ColumnAnalysis],
        rows: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Detect executive/business metrics in the data."""
        detected = {}

        for col in columns:
            col_lower = col.name.lower()

            for metric_type, patterns in self.EXECUTIVE_METRIC_PATTERNS.items():
                if any(p in col_lower for p in patterns):
                    if col.dtype == 'numeric' and col.mean_value is not None:
                        detected[metric_type] = {
                            ...
                            'min': col.min_value,
                            'max': col.max_value,
                            'total': sum(float(row.get(col.name, 0) or 0) for row in rows)
                        }
                    else:
                        # Count status values if present
    ...
    def _generate_insights(
        ...
        if detected_metrics:
            ...
            insights.append(f"Detected business data: {', '.join(metric_names)}")

        # Revenue insight
        if 'revenue' in detected_metrics:
            rev = detected_metrics['revenue']
            if 'total' in rev:
                insights.append(f"Total revenue in dataset: ${rev['total']:,.2f}")

        # Goals status insight
        if 'status' in detected_metrics:
            dist = detected_metrics['status'].get('distribution', {})
            if dist:
                on_track = dist.get('on track', 0) + dist.get('complete', 0)
                at_risk = dist.get('at risk', 0) + dist.get('behind', 0)
                if on_track > 0 or at_risk > 0:
                    insights.append(f"Status: {on_track} on track, {at_risk} at risk/behind")
    ...
    def _create_summary(
        ...
        if detected_metrics:
            parts.append(f"Business data: {', '.join(detected_metrics.keys())}")
