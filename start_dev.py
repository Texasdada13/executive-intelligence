#!/usr/bin/env python
"""Development Server Startup Script for Executive Intelligence"""
import os
import sys
import socket
import random
...
def find_free_port(start=5103, end=5199):
...
def load_demo_data():
    ...
    with app.app_context():
        if Organization.query.count() == 0:
            print("📈 Loading demo executive data...")
            org_id = load_demo_data_to_db(db, models, "TechVenture Corp")
...
def main():
    ...
    print("=" * 60)
    print("  Executive Intelligence - Development Server")
    print("=" * 60)

    # Use assigned port (5103)
    port = 5103
    print(f"\n🔌 Using port: {port}")
